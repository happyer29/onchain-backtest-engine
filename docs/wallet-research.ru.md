...
Полный граф открывается кнопкой «Весь результат»: до 200 000 пар и 5 000
участвующих кошельков, независимо от страницы таблицы. Три уровня — все группы,
кошельки группы и все соседи кошелька, в том числе из других групп. Межгрупповая
линия означает число пар кошельков, линия между кошельками — одну пару с числом
общих токенов. Группы — визуальная эвристика, не доказательство общего владельца.

«Показывать пары: минимум общих токенов» — фильтр отображения. Например, 5
скрывает пары с меньшим числом общих токенов. Анализ, состав групп и таблицы
не меняются; показанные + скрытые пары равны полному числу. Межгрупповые счётчики
и списки пар учитывают тот же фильтр. Все кошельки остаются доступны в поиске.
«Показать все связи» сбрасывает фильтр без повторной загрузки. Это отдельная
настройка от режима Mayhem, который по-прежнему требует нового анализа.

ForceAtlas2 размещает узлы по связям в отдельном worker: до 120 итераций и
30 секунд вместе с построением вида. Приближение раскрывает подписи; наведение
подсвечивает соседей. «Назад к предыдущему виду» возвращает сохранённые
координаты и масштаб: хранятся 8 последних видов и 16 шагов истории, без копий
рёбер или неактивных графов. Более старый вид строится заново. Смена результата
или области очищает эту память. «На весь экран» открывает рабочую область графа.
При ошибке worker/WebGL, таймауте или отмене доступен повтор, а таблицы и
исходные покупки остаются доступны.

Граф на локальной Sigma.js/React Sigma поддерживает масштаб колесом/жестом,
...
Область графа выбирается отдельно от страницы таблицы. Более крупный или
плотный снимок может превысить лимиты ресурсов; сузь диапазон или явно выбери
подписантов.
