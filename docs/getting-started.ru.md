...
![Форма Copy Buy на английском языке в светлой теме, изолированные тестовые данные](assets/launch-strategy.png)

Пошаговое объяснение подготовки данных, очереди, результатов, ML,
исследований, артефактов и ресурсов приведено в [руководстве по Web UI](ui-guide.ru.md).

Для точного ML на одном ReplayPack откройте **Models and features** и по
порядку опубликуйте FeatureSet, Universe, LabelSet, обученную модель и
ModelSchedule. Первое `eligible_from` должно быть не раньше доступности
модели, а все поздние решения должны быть покрыты. В **Predictions** выберите
«Без прогноза до первой модели»: ранние строки останутся без прогноза в
PredictionSet v2. Для FirstSwap с этим PredictionSet выберите `FROZEN`, то же
правило префикса и `NULL` в поле «Отсутствующий прогноз», если ранние строки
нужно пропустить. Модель остаётся привязанной к тому же FeatureSet, на котором
обучена; внутренние и конечные пробелы расписания — ошибка.

Короткий [видеообзор с русской озвучкой](assets/ml-workflow-demo-ru.mp4)
показывает интерфейс и это правило на синтетических данных. В ролике нет
подключения реального источника и полного запуска ML-задачи через UI.
