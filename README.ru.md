...
На изображениях результатов стратегии используются изолированные browser fixtures, а не ваш рабочий
...
### Граф связей кошельков

![Граф Sigma с 78 группами кошельков, фильтром связей и переходом к отдельным группам](docs/assets/research-graph.png)

Обзор сохранённого исследования: 150 233 пары и 2 752 кошелька. Можно открыть
группу, изучить окружение кошелька или скрыть слабые связи по числу общих токенов.
Граф показывает наблюдаемые совместные покупки; связь не доказывает общего владельца.

## Локальное статическое демо

Общий React-интерфейс графа и результатов можно посмотреть на 60 тестовых
кошельках, 423/426 связях и двух примерах Copy Buy, рассчитанных заранее.
При просмотре готового сайта источник и Python-сервер не нужны.
[Подготовка демо и ручная публикация в GitHub Pages](docs/demo.ru.md).
Подготовка ничего не публикует.
...
Sigma.js/React Sigma позволяет масштабировать граф, перемещать узлы, выбирать связи
и открывать покупки пары. Доступны текущая страница (25 пар / 50 узлов) и весь
результат (до 200 000 пар / 5 000 кошельков): группы → кошельки группы →
окружение кошелька. Фильтр отображения по числу общих токенов сохраняет
состав групп и анализ; счётчики показывают видимые/скрытые пары. Раскладка
работает в ограниченном worker, а возврат восстанавливает масштаб и позиции. Начни с
